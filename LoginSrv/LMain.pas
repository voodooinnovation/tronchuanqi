...
begin
  ...
  SendGameCenterMsg(SG_FORMHANDLE, IntToStr(Self.Handle));//发送主窗口句柄

  Config := @g_Config;
  StartService(Config); //开启服务
...
begin
  ...
  try
    for I := 0 to g_MainMsgList.Count - 1 do begin //从缓存里加到memo里
      Memo1.Lines.Add(g_MainMsgList.Strings[I]);
    end;
    g_MainMsgList.Clear; //清除缓存
  finally
...
var
  Config: pTConfig;
begin
  StartTimer.Enabled := False;//自启动时钟关闭
  Config := @g_Config;
