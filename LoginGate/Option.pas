...
procedure TfrmOption.Open();//读取配置
var
  I: Integer;
  sIPaddr: string;
begin
  ListBoxActiveList.Clear;  //当前连接
  ListBoxBlockList.Clear;   //永久过滤
  ListBoxIPArrayList.Clear; //ip段
  if frmMain.ServerSocket.Active then //是否启用sock
    for I := 0 to frmMain.ServerSocket.Socket.ActiveConnections - 1 do begin
      sIPaddr := frmMain.ServerSocket.Socket.Connections[I].RemoteAddress; //获取客户端IP
      if sIPaddr <> '' then
      //添加动态列表
        ListBoxActiveList.Items.AddObject(sIPaddr, TObject(frmMain.ServerSocket.Socket.Connections[I]));
    end;
