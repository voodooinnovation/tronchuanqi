...
type
  ...
  TfrmMain = class(TForm)
    ...
    procedure MENU_VIEW_SENDMSGClick(Sender: TObject);
  private
...
{$IFEND}
  g_boClose := False;//标记关闭按钮
  Application.OnException := OnProgramException;//错误事件 发生错误 现实到日志中

  g_dwGameCenterHandle := Str_ToInt(ParamStr(1), 0);//游戏控制中心句柄
  nX := Str_ToInt(ParamStr(2), -1);//坐标
  nY := Str_ToInt(ParamStr(3), -1);//坐标
  if (nX >= 0) or (nY >= 0) then begin
    Left := nX;
    Top := nY;
  end;
  SendGameCenterMsg(SG_FORMHANDLE{标记发送程序句柄}, IntToStr(Self.Handle));//向游戏中心发送消息
  ...
  g_boShowLog := MENU_VIEW_LOGMSG.Checked;//是否显示日志
  g_Config.boStarted := False;//标记服务是否已启动

  g_SessionList := TSessionList.Create;//回话列表
  //g_SessionList.ItemSize := SizeOf(TSession);

  g_QuickList := TQuickList.Create;
  g_CCAttackIPList := TGStringList.Create;//cc攻击ip列表
  g_IPList := TGStringList.Create;//ip列表
  g_BlockIPList := TGStringList.Create;//禁止ip
  g_BlockIPArray := TStringList.Create;//禁止ip段
  RzPanel.DoubleBuffered := True;//使用辅助缓冲避免闪烁
  SendGameCenterMsg(SG_STARTNOW{标记开始启动}, g_sNowStartGate{正在启动登陆网关});
  TimerStart.Enabled := True;//启动时钟
end;
...
begin
  try
    LoadConfig;//载入配置信息
    Caption := g_Config.GateName + ' - ' + g_Config.TitleName;//设置标题
    g_SessionList.MaxCount := g_Config.nMaxCount;//会话列表最大数

    g_nNewAccountCount := 0;//新用户
    g_dwNewAccountTick := GetTickCount();
    g_dwClearBlockIPListTick := GetTickCount();
    g_dwStartServiceTick := GetTickCount();
    g_boStartService := False;//未启动

    g_boOldCheckAttack := False;//老的检查攻击
    g_nAttackCount := 0;//攻击数
    g_dwLastAttackTick := GetTickCount();

    g_nSessionCount := 0;
    g_boGateReady := False;//网关未准备好
    g_boServerReady := False;//未准备
    MENU_CONTROL_OPENATTACK.Checked := g_boCheckAttack; //读取配置
    g_ClientThread := TClientThread.Create(True);//客户端线程创建  暂停
    g_ClientThread.Resume;//启动现成

    ServerSocket.Port := g_Config.GatePort; { Use telnet port   }//网关端口
    ServerSocket.Address := g_Config.GateAddr; { Use any interface }//网关地址
    ServerSocket.Active := True;//启动网关
    g_boServiceStart := True;//服务已启动
    g_boServerReady := True;//服务已准备
   ...
   { 这只客户端
    ...
    Timer.Enabled := True;//启动timer时钟
    MainOutMessage('启动服务完成...', nil);//启动完成

    g_Config.boStarted := True; //已启动
    g_boClose := False; //不在关闭状态

    MENU_START.Enabled := False;
    MENU_STOP.Enabled := True;
    SendGameCenterMsg(SG_STARTOK, g_sNowStartOK);//向控制中心发送 启动成功
    //HTTPGetIpList();//获取绿色通道  貌似无用 没写完
    ...
    on E: Exception do begin
      ...
      MainOutMessage(E.Message, nil);//输出错误
    end;
  end;
end;
...
begin
  ...
  g_ClientThread.Terminate; //结束客户端
  FreeAndNil(g_ClientThread); //释放

  SaveBlockIPFile;  //保存禁止ip
  SaveBlockIPArrayFile;//保存禁止ip段
  MENU_START.Enabled := True;
  MENU_STOP.Enabled := False;
...
begin
  TimerStart.Enabled := False; //停止时钟
  if not g_Config.boStarted then begin //如果已启动服务则停止服务 否则开启服务
    StartService;
  end else begin
    StopService;
    if g_boClose then begin  //如果关闭标记为真  释放所有内存空间
...
begin
  ...
  sData := StrPas(MsgData.CopyDataStruct^.lpData);//复制数据结构
  case wIdent of //消息代码
    GS_QUIT: begin
        Caption := g_Config.TitleName + ' [正在关闭服务...]';
...
procedure TfrmMain.MENU_VIEW_SENDMSGClick(Sender: TObject);
begin
MENU_VIEW_SENDMSG.Checked := not MENU_VIEW_SENDMSG.Checked;
end;

procedure TfrmMain.MENU_OPTION_GENERALClick(Sender: TObject);//打开设置时
begin
  frmGeneralConfig := TfrmGeneralConfig.Create(Owner);
  frmGeneralConfig.Top := Self.Top + 28;
  frmGeneralConfig.Left := Self.Left;
  frmGeneralConfig.Open();//读取配置
...
begin
  ...
  if g_boGateReady then begin
    sRemoteIPaddr := Socket.RemoteAddress;//远程IP
    ...
    if ServerSocket.Socket.ActiveConnections >= g_Config.nMaxOnlineCount then begin
      for nIdx := ServerSocket.Socket.ActiveConnections - 1 downto 0 do begin
        ...
        if not IsUserIPList(sRemoteIPaddr) then begin  //判断是否在绿色通道中
...
begin
  Session := g_SessionList.Items[Socket.nIndex];
  if Session <> nil then begin
    sReviceMsg := Socket.ReceiveText;//获取内容
    if g_boCheckAttack then begin //是否开启攻击保护
      if not IsUserIPList(Session.sRemoteIPaddr) then begin //如果不在用户列表
        if Length(sReviceMsg) > 355 then begin //如果数据大于355则关闭客户端
