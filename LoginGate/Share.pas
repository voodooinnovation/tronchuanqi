...
resourcestring
  g_sUpDateTime = '更新日期: 2011/01/23';
  g_sProductName = 'MirYQ防攻击登陆网关 V 3.0';
  g_sProgram = '程序制作: MirYQ QQ:442517066';
  g_sWebSite = '程序网站: http://www.MirYQ.com';
...
type
  ...
  TClientThread = class(TThread) //客户端线程
    ClientSocket: TClientSocket; //通讯控件
    m_UserCriticalSection: TRTLCriticalSection;//多线程
...
var
  ...
  GateClass: string = 'LoginGate';//网关类型
  ...
  g_IPList: TGStringList;//绿色通道
  g_BlockIPList: TGStringList;//内存中标记禁止IP
  g_BlockIPArray: TStringList;//禁止ip段
  ...
  g_sIpListUrl: string = 'http://www.MirYQ.com/IpList.txt';
...
begin
  ...
  ClientSocket.OnConnect := ClientSocketConnect;//连接事件
  ClientSocket.OnDisconnect := ClientSocketDisconnect; //断开连接事件
  ClientSocket.OnError := ClientSocketError;//错误事件
  m_boConnected := False;//未连接
  m_sReviceText := '';//收到字符串
...
resourcestring
  sExceptionMsg = '[Exception] TClientThread::Execute %d';//资源串
var
  nMsgLen: Integer;
  nCheckCode: Integer;
...
begin
  while not Terminated do begin
    //try
    nCheckCode := 0;
    if g_boServiceStart then begin //判断服务是否启动
      if not ClientSocket.Socket.Connected then begin //如果未连接
        m_boConnected := False;
        if GetTickCount - m_dwConnTick > 1000 then begin
          m_dwConnTick := GetTickCount;
          nCheckCode := 1;
          ClientSocket.Active := False; //关闭
          ClientSocket.Address := g_Config.ServerAddr; //服务器ip
          ClientSocket.Port := g_Config.ServerPort;   //服务器端口
          try
            ClientSocket.Active := True;//开启
          except
            Sleep(1000);
            Continue;
          end;
      ...
      Connect();//连接服务器
      nCheckCode := 4;
      SocketStream := TWinSocketStream.Create(ClientSocket.Socket, 5000);//创建sock流
      try
        if SocketStream.WaitForData(5000) then begin
          repeat
            ...
            try
              nCheckCode := 5;
              nMsgLen := SocketStream.Read(ReceiveBuffer{读取数据}, SizeOf(ReceiveBuffer));
              nCheckCode := 6;
            except
              Close();
              Break;
            ...
            if nMsgLen <> 0 then begin
              ...
              Move(ReceiveBuffer, sReviceBuff[1], nMsgLen); //转换文本
              m_sReviceText := m_sReviceText + sReviceBuff;
              nCheckCode := 8;
              while (True) do begin
                if m_sReviceText = '' then Break;
                if Length(m_sReviceText) < 2 then Break;
                if Pos('$', m_sReviceText) <= 0 then Break;//如果找不到 则跳出循环
                sSocketMsg := '';
                m_sReviceText := ArrestStringEx(m_sReviceText, '%', '$', sSocketMsg);//格式化包
                if sSocketMsg = '' then Continue;
                if Terminated then Break;
                nCheckCode := 9;
                ProcessPacket(sSocketMsg); //发送包
                nCheckCode := 10;
              end;
            end else begin
              Break;
    ...
    if (sSocketMsg[1] = '+') then begin
      ...
      try
        ...
        if (Session <> nil) then begin
          nCheckCode := 11;
          if (Session.Socket <> nil) and Session.Socket.Connected then begin
            Session.SendMsgList.Add(sSocketMsg);//发送数据
          end;
          nCheckCode := 12;
        end;
      finally
...
var
  I: Integer;
begin
  MainLogMsgList.Lock;//锁定
  try
    for I := 0 to MainLogMsgList.Count - 1 do begin
      Dispose(pTMsgLog(MainLogMsgList.Objects[I]));//释放内存
    end;
...
var
  MsgLog: pTMsgLog;
begin
  //g_boShowLog  是否显示消息 由菜单中 查看消息负责
  if g_boShowLog then begin
    MainLogMsgList.Lock;//锁定日志消息列表
    try
      if MainLogMsgList.Count >= 200 then ClearMessage; //如果超过200行就清空
    finally
      MainLogMsgList.UnLock;
    end;
    MsgLog := FindMsg(AObject);
    if MsgLog = nil then begin //对象是否为空
      MainLogMsgList.Lock;
      try
        New(MsgLog);//分配内存一个结构指针
        MsgLog.dAddTime := Now;//添加时间
        MsgLog.boAdd := True;//添加
        MsgLog.boUpDate := False;//更新
        MsgLog.APointer := Pointer(AObject);//指针
        MainLogMsgList.AddObject(Msg, TObject(MsgLog));//添加对象和消息
      finally
        MainLogMsgList.UnLock;
      end;
    end else begin
      MsgLog.dAddTime := Now;//添加时间
      MsgLog.boUpDate := True;//更新日期
    end;
  end;
end;
...
begin
  ...
  try
    ...
    if FileExists(sFileName) then begin
      LoadList := TStringList.Create;
      LoadList.LoadFromFile(sFileName);
      for I := 0 to LoadList.Count - 1 do begin //去除空格并加入禁止列表
        sIPaddr := Trim(LoadList.Strings[I]);
        if sIPaddr = '' then Continue;
        AddBlockIP(sIPaddr);
      end;
      ...
      for I := g_IPList.Count - 1 downto 0 do begin
        if I > 0 then begin
          for II := I - 1 downto 0 do begin
            if g_IPList.Strings[I] = g_IPList.Strings[II] then begin //判断上一个IP是否重复
              g_IPList.Delete(I);//重复则删除
              Break;
            end;
          end;
        end;
