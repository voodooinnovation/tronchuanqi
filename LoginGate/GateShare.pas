...
begin
  Result := -1;
  sReceiveMsg := Msg;
  if (Pos('!', sReceiveMsg) > 0) and (Length(sReceiveMsg) >= 2) then begin //查找字符串含有!并且大于2
    sReceiveMsg := ArrestStringEx(sReceiveMsg, '#', '!', sMsg);//分割字符串
    if sMsg <> '' then begin
      if Length(sMsg) >= DEFBLOCKSIZE + 1 then begin
        ...
        DefMsg := DecodeMessage(sDefMsg);//解密
        Result := DefMsg.ident;
      end;
    end;
  end;
