...
function MakeDefaultMsg(wIdent: Word; nRecog: Integer; wParam, wTag, wSeries: Word): TDefaultMessage;
begin
  Result.Recog := nRecog;//整数型 第二个参数
  Result.Ident := wIdent;//0-65535 16位无符号整型 识别
  Result.Param := wParam;//0-65535 16位无符号整型 参数
  Result.Tag := wTag;//0-65535 16位无符号整型  标识
  Result.Series := wSeries;//0-65535 16位无符号整型 系列
end;
...
var
  EncBuf, TempBuf: array[0..BUFFERSIZE - 1] of Char; //字符型的缓冲区
begin
  Move(sMsg, TempBuf, SizeOf(TDefaultMessage));//从sMsg中移动到TempBuf中
  Encode6BitBuf(@TempBuf, @EncBuf, SizeOf(TDefaultMessage), SizeOf(EncBuf));//将缓存中移入编码后的缓存
  Result := StrPas(EncBuf);//将pchar转换为 字符串
end;
