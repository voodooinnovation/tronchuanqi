...
uses
  ...
  MapUnit, SoundUtil, ComCtrls, EncryptUnit, Share, Wzl, ExtCtrls, RzPanel,DXDraws;
...
type
  ...
  TFrmDlg = class(TForm)
    ...
    procedure DSPage1MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure DSPage2MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure DSPage3MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure DSPage4MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
  private
    DlgTemp: TList;
    ...
    if Pos('@http://', sClickString) > 0 then begin
      ...
      if (sMsg <> '') and (Str <> '') then begin
        ...
        g_sDownSongAddr := sMsg;
          ...
          MediaStartPlay(g_sDownSongAddr, False, True);
...
procedure TFrmDlg.HideAllControls;//隐藏所有控件
var
  I: Integer;
  c: TControl;
begin
...
procedure TFrmDlg.RestoreHideControls;//还原控件
var
  I: Integer;
  c: TControl;
begin
...
procedure TFrmDlg.SetBookType(nType: Integer);//读取书
var
  d: TTexture;
begin
  DBook.Visible := False;//不显示
  g_nBookType := nType;//页码
  g_nBookPage := 0;
  ...
  DBook.Visible := True;//显示
end;

procedure TFrmDlg.Initialize;//初始化
var
  d: TTexture;
...
begin
  ...
  if Initialized then Exit;//是否初始化过
...
{-------------------------------------------------------------------------------}
  ...
  //人物状态窗口
    d := g_WMainImages.Images[806]; //惑怕  370
    if d <> nil then begin
      DStateWin.SetImgIndex(g_WMainImages, 806);
...
function TFrmDlg.AddMemoChat(Str: string; FColor, BColor: Integer): string;//添加聊天文字
var
...
begin
  if g_boItemMoving then begin
    ...
    if g_MovingItem.Item.s.Name = g_sGoldName {'金币'} then begin
      ...
      frmMain.SendDropGold(dropgold);//发送丢掉金币
    end;
    ...
    with Sender as TDButton do begin
     ...
     case nStatus of    //服务器状态
      ...
      if d <> nil then begin
        ...
        if TDButton(Sender).Downed then begin  //判断按钮是否按下
...
begin
  case g_ServerList.Count of  //如果有几个 显示几个服务器登陆框
    1: begin
...
procedure TFrmDlg.DSServer1Click(Sender: TObject; X, Y: Integer);//选择服务器按钮
var
  svname: string;
begin
  svname := '';
  ...
  if svname <> '' then begin
    if BO_FOR_TEST then begin  //测试模式
      svname := 'MakeGM';
      g_sServerMiniName := 'MakeGM';
    end;
    frmMain.SendSelectServer(svname);//发送选择服务器
    DSelServerDlg.Visible := False; //隐藏服务器选择画面
    g_sServerName := svname;//服务器名称
    //Showmessage(svname);
  end;
end;
  ...
  with DStateWin do begin
    ...
    case StatePage of
      0: begin //人物界面
          ...
          if d <> nil then
          dsurface.Draw(Left+3, top+61, d.ClientRect, d, True);
          //左边字
          MainForm.Canvas.Font.Style:=[fsBold];
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 70,'装', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 83,'备', clBlack, clBlack );
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 72,'装', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 85,'备', clBlack, clBlack );
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 71,'装', $00A6E1EE, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 84,'备', $00A6E1EE, clBlack );

          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 117,'状', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 130,'态', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 119,'状', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 132,'态', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 118,'状', $006C85AA , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 131,'态', $006C85AA , clBlack);

          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 162, '属', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 175,'性', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 164, '属', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 177,'性', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 163, '属', $006C85AA , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 176,'性', $006C85AA , clBlack);

          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 207,'技', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 220,'能', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 209,'技', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 222,'能', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 208,'技', $006C85AA , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 221,'能', $006C85AA , clBlack);

          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 252,'待', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 265,'出', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 254,'待', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 267,'出', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 253,'待', $006C85AA , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 266,'出', $006C85AA , clBlack);
          MainForm.Canvas.Font.Style:=[];
        end;
      1: begin //状态
          //左边按钮图片
          d := g_WMainImages.Images[793];
          if d <> nil then
          dsurface.Draw(Left+3, top+61, d.ClientRect, d, True);
          //左边的字
          MainForm.Canvas.Font.Style:=[fsBold];
          ...
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 72,'装', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 85,'备', clBlack, clBlack );
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 71,'装', $006C85AA, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 84,'备', $006C85AA, clBlack );

          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 117,'状', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 9, SurfaceY(Top) + 130,'态', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 119,'状', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 132,'态', clBlack, clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 118,'状', $00A6E1EE , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 131,'态', $00A6E1EE , clBlack);
      ...
      2: begin //属性
          ...
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 164, '属', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 177,'性', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 163, '属', $00A6E1EE , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 176,'性', $00A6E1EE , clBlack);
      ...
      3: begin //魔法
          ...
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 209,'技', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 11, SurfaceY(Top) + 222,'能', clBlack , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 208,'技', $00A6E1EE , clBlack);
          dsurface.BoldTextOut(SurfaceX(Left) + 10, SurfaceY(Top) + 221,'能', $00A6E1EE , clBlack);
...
procedure TFrmDlg.DStateWinMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  DScreen.ClearHint;//清楚提示
  HintList.Clear;//清除提示
  g_MouseStateItem.s.Name := ''; //鼠标状态物品
  g_MouseHeroItem.s.Name := '';  //鼠标英雄物品
end;
...
procedure TFrmDlg.DSPage1MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
    StatePage := 0;
    PageChanged;
...
procedure TFrmDlg.DSPage2MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
    StatePage := 1;
    PageChanged;
...
procedure TFrmDlg.DSPage3MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
    StatePage := 2;
    PageChanged;
...
procedure TFrmDlg.DSPage4MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
    StatePage := 3;
    PageChanged;
...
begin
  ...
  case nIndex of
    ...
    1: begin
        ...
        MediaStartPlay(Trim(sItemData), False, True);
      end;
    2: begin
        sItemData := TDLabel(Sender).TData;
        BrowserNavigate(Trim(sItemData));
      end;
  end;
end;
