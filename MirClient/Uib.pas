...
type
  ...
  private
    procedure LoadDxImage(const AFileName: string; DXImage: pTDXImage);//载入图片
    function GetTextureByName(Name: string): TTexture;//获得texture
    function GetIndexByName(Name: string): Integer;//获得索引
  protected
...
begin
  inherited;
  m_FileList := TStringList.Create;//创建
end;
...
procedure TUibImages.Initialize;//初始化
begin
  if not Initialized then begin //判断是否已初始化
    BitCount := 8;//位计数
    ImageCount := m_FileList.Count;//图片数
    //FileName := 'Data\';
    m_ImgArr := AllocMem(SizeOf(TDXImage) * ImageCount);//分配内存 置0
    Initialized := True;
  end;
end;
...
begin
  if Initialized then begin
    Initialized := False;
    IndexList.Clear;//清理
    if m_ImgArr <> nil then begin  //释放所有对象
