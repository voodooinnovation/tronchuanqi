...
type
  ...
  public
    ...
    procedure Initialize; dynamic;//虚方法
  ...
  TIntroScene = class(TScene)//继承场景类  开场场景
  private
  public
    constructor Create;
    destructor Destroy; override;  //重载
    procedure OpenScene; override; //打开场景           空方法
    procedure CloseScene; override; //关闭场景
    procedure PlayScene(MSurface: TTexture); override; //玩场景
  end;

  TLoginScene = class(TScene) //登陆场景
  private
   { m_EdId: TEdit;
    m_EdPasswd: TEdit;}
    m_EdNewId: TEdit;
  ...
  public
    m_sLoginId: string;//登陆id
    m_sLoginPasswd: string;//登陆密码
    m_boUpdateAccountMode: Boolean;//更新帐户模式
    constructor Create;
  ...
  TSelectChrScene = class(TScene)//选择角色场景
  private
    SoundTimer: TTimer;
...
implementation
uses
  ClMain, MShare, Share, SoundUtil, DXSounds, FState;
...
begin
 ...
 { m_EdId := TEdit.Create(frmMain.Owner);
  ...
  m_EdSSNo := TEdit.Create(frmMain.Owner); //证件号码
...
procedure TLoginScene.OpenScene;//打开登陆场景
var
  I: Integer;
  d: TTexture;
begin
  ...
  HideLoginBox;//隐藏所有对话框
  PlayBGM(bmg_intro);//播放音乐
end;
...
begin
  ...
  if Key = #13 then begin
    Key := #0;
    m_sLoginId := LowerCase(FrmDlg.EdId.Text);//帐号改变为大写
...
procedure TLoginScene.PassWdFail;//密码错误
begin
  FrmDlg.EdId.SetFocus;
end;

function TLoginScene.NewIdCheckNewId: Boolean;//检查用户id
begin
  Result := True;
...
function TLoginScene.NewIdCheckSSno: Boolean; //检查证件号码
var
...
function TLoginScene.NewIdCheckBirthDay: Boolean;//检查生日
var
  Str, t1, t2, t3, syear, smon, sday: string;
  ayear, amon, aday, sex: Integer;
  flag: Boolean;
...
begin
  ...
  HideLoginBox;//隐藏登陆窗口
  PlaySound(s_rock_door_open);//播放声音
end;
...
{$IFEND}
  //Exit;
  if d <> nil then begin
    MSurface.Draw((SCREENWIDTH - 800) div 2, (SCREENHEIGHT - 600) div 2, d.ClientRect, d, False);//画背景
  end;

  if m_boNowOpening then begin

    //开门速度
    Inc(m_nCurFrame);//开门动画
...
{$IFEND}
    ...
    if g_boDoFadeOut then begin
      if g_nFadeIndex <= 1 then begin
        ...
        DScreen.ChangeScene(stSelectChr);//改变场景
...
begin
 ...
 //注册和修改密码的时候 显示所有编辑框 更具控件tag判断哪些显示哪些不显示
...
begin
  ...
  ChangeLoginState(lsNewid);//改变为新建用户
end;
...
begin
  ...
  SelDelChar := -1;
  FillChar(DelCharArray, SizeOf(TDelChar) * 10, #0);//删除的人物
...
begin
  ...
  if FileExists(sFileName) then begin
    ...
    try
      LoadList.LoadFromFile(sFileName);//载入文件
      for nIndex := LoadList.Count - 1 downto 0 do begin //到循环
        if Trim(LoadList[nIndex]) = '' then LoadList.Delete(nIndex);//如果为空就删除
      end;
    except

    end;

    nIndex := Random(LoadList.Count - 1); //随机选取一个
    if (nIndex >= 0) and (nIndex < LoadList.Count) then begin
      m_sTipMsg := LoadList[nIndex];//获取提示信息
    end;
    LoadList.Free;
  end;
...
procedure TSelectChrScene.PlayScene(MSurface: TTexture);//选择人物效果
var
