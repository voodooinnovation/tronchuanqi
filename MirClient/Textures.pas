...
type
  ...
  TTextureDevice = class(TComponent) //控件
  protected
...
procedure Register;
begin
  RegisterComponents('MirDevice', [TTextureDevice]); //注册控件组
end;
...
begin
  ...
  CPU := TCPU.Create;
  CPU.GetInfo();
  IsMMX := CPU.Features.MMX;//是否支持mmx
  IsSSE := CPU.Features.SSE and CPU.Features.SSE2;//是否支持sse sse2
