...
uses
   Windows, SysUtils, Classes, Graphics, wmUnit, GameImages, Textures, DIB, HUtil32, ZlibEx, CompressUnit, CompressUnit1;
  ...
  try
   ...
   case ImageInfo.m_Enc1 of
    3:begin
      ...
      DXImage.nPx := ImageInfo.m_wPx;
      DXImage.nPy := ImageInfo.m_wpy;
      DXImage.dwLatestTime := GetTickCount;

      GetMem(S, WidthBytes(ImageInfo.m_nWidth) *ImageInfo.m_nHeight);//实际大小
    ...
    try
      ...
      if   ImageInfo.m_Len >0  then begin
        ...
        Move(SrcP^, DesP^, ImageInfo.m_Len);
      end;       
      FreeMem(SrcP);
      SrcP := S;
      ...
      for I := 0 to DXImage.Texture.Height - 1 do begin //256色数据转换成16位数据
          ...
          //SrcP := PByte(Integer(S) + I * ImageInfo.m_nWidth);
    ...
    5:begin
      ...
      DXImage.nPy := ImageInfo.m_wpy;
      DXImage.dwLatestTime := GetTickCount;
       GetMem(S, ImageInfo.m_nWidth *ImageInfo.m_nHeight * 2);//实际大小
       GetMem(SrcP, ImageInfo.m_Len);//压缩源大小
    try
      DesP := S;//实际大小
      m_FileStream.Read(SrcP^, ImageInfo.m_Len);//读压缩数据
