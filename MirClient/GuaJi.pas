...
type
  ...
  TGuaJi = class  //无继承
    m_TargetCret: TActor;{自定义类型 精灵属性}
    m_nTargetX: Integer; //目标座标x
    m_nTargetY: Integer; //目标座标t
  private
    FPath: TPath;//路径数组 动态的
    FPathPoisonIndex: Integer;
    FStarted: Boolean;
    FRunTick: LongWord;
    FAttackTick: LongWord;
    function CanRunEx(sx, sY, ex, ey: Integer; Flag: Boolean): Boolean;//是否能跑
    function CanWalkEx(nTargetX, nTargetY: Integer; Flag: Boolean): Boolean;//是否能走扩展
    function CanWalk(nTargetX, nTargetY: Integer; Flag: Boolean): Boolean;//是否能走
    ...
    procedure GetAutoWalkXY(var nTargetX, nTargetY: Integer);//自动走坐标
    function GetNextPosition(sX, sY, nDir, nFlag: Integer; var snx: Integer; var sny: Integer): Boolean;//下一个坐标
  public
    constructor Create;
...
begin
  ...
  if g_Config.boGuaji{全局变量 挂机是否开启} and (g_MySelf{自己 THumActor 人物精灵} <> nil) and (not g_MySelf.m_boDeath{是否死亡}) and (not FStarted) then begin
    FStarted := True;
    FRunTick := GetTickCount;
    DScreen.AddChatBoardString('开始挂机 Ctrl + L', clYellow, clRed);//聊天对话框加入字
  end;
end;
