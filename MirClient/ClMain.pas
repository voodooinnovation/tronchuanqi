...
uses
  ...
  Grobal2, DIB, StdCtrls, Wzl, HUtil32, EncryptUnit, Actor, Mpeg, MapUnit,
  ...
  SoundEngn, ShellApi, tlHelp32, psAPI, GameImages,
...
procedure BrowserNavigate(const URL: WideString);
procedure MediaStartPlay(const FileName: string; boShow, boPlay: Boolean);
procedure MediaStopPlay(const FileName: string);
...
uses FState, Progress, WelCome,DlgConfig;
...
procedure MediaStartPlay(const FileName: string; boShow, boPlay: Boolean);
begin
//  if frmCqFirMediaPlayer <> nil then
//    frmCqFirMediaPlayer.Play(FileName, boShow, boPlay);
end;
...
begin
  Result := 0;
  if ((wParam = 9{tab}) {or (WParam = 13回车)}) and (g_nLastHookKey = 18{alt}) and (GetTickCount - g_dwLastHookKeyTime < 500) then begin
...
begin
  ...
  StrCopy(SendData.lpData, PChar(sSendMsg));//将字符串拷贝到内存
  SendMessage(g_dwGameLoginHandle{句柄}, WM_COPYDATA{类型}, nParam{参数}, Cardinal(@SendData){地址指针});//向登录器发送消息
  FreeMem(SendData.lpData);//释放内存
end;
...
begin
  DebugOutStr('----------------------- start ------------------------');
  GetWindowThreadProcessId(Handle, @g_nSelfThreadProcessId);//获取进程ID
  g_sAppFilePath := ExtractFilePath(Application.ExeName);//程序路径
  g_ConfigClient.btMainInterface := 1; //1 征途界面 2 盛大界面
  FInterval2 := 1;
  FInterval := 50;
  FOldTime := TimeGetTime;  //以毫秒计算机系统时间，获取开机到现在的时间
  FOldTime2 := TimeGetTime;
  //FrmDlg.DBackground.Parent:=Self;
{$IF CLIENTTEST = 0}
  g_dwGameLoginHandle := Str_ToInt(ParamStr(1), 0);//第一个参数 游戏登录器句柄
  g_sMainParam2 := ParamStr(2); //读取设置参数  第二个参数 游戏配置

  DecodeBuffer(g_sMainParam2, @MirServer, SizeOf(TMirServer));//解密配置文件

  g_boFullScreen := MirServer.boFullScreen;//设置全屏
  g_sServerAddr := MirServer.sServeraddr; //设置游戏地址
  g_nServerPort := MirServer.nServerPort; //设置游戏端口

  SCREENWIDTH := MirServer.nScreenWidth;//设置屏幕宽
  SCREENHEIGHT := MirServer.nScreenHegiht; //设置屏幕高
{$ELSE}
  g_boFullScreen := false;
  SCREENWIDTH := 800;//1024
  SCREENHEIGHT := 600;//768
   // g_sServerAddr := '222.35.77.233';//测试服务器
 // g_nServerPort := 7956;
  ...
  if ini <> nil then begin
   // if EnglishVersion then begin
    ...
    g_boFullScreen := true;//ini.ReadBool('Setup', 'FullScreen', g_boFullScreen);
...
{$IFEND}
  MAPSURFACEWIDTH := SCREENWIDTH;//地体表面宽
  if (g_ConfigClient.btMainInterface = 1) or (SCREENWIDTH = 1024) then begin  //判断界面
 ...
 // frmCqFirMediaPlayer := nil;//播放器
//  frmOpenDiaLogNew := nil; //播放器_打开新文件
  frmWelCome := nil;//游览器

  g_dwShowVersionBmpTick := GetTickCount;//操作系统启动到现在经过的毫秒数

  MainForm := Self;//textures 的变量
  Application.OnMessage := AppMessage;

  ImageCanvas.SetSize(SCREENWIDTH, SCREENHEIGHT);//texture的类 设置分辨率

  g_BackSurface := TTexture.Create;
  g_BackSurface.SetSize(1, 1);

  BorderStyle := bsSingle; //设置样式
  BorderIcons := [biSystemMenu, biMinimize];//设置标题按钮

  SendGameCenterMsg(CM_HANDLE, IntToStr(Handle));//向登录器发送自己的句柄

  ClientWidth := SCREENWIDTH;//设置内边框
  ClientHeight := SCREENHEIGHT;//外边框

  Application.OnMessage := AppMessage;
  Canvas.Font.OnChange := FontChange; //字体改变 代码无效

  g_GuaJi := TGuaJi.Create; //挂机类

  g_UnbindItemList := TList.Create;//可以能帮物品列表
  g_UnbindItemFileList := TStringList.Create;//捆绑物品文件
  g_ShowItemFileList := TStringList.Create;//显示物品文件列表
  g_ShowItemList := TFileItemDB.Create;//显示物品列表
  g_MediaList := TGStringList.Create;//播放列表

  g_ItemDescList := TGStringList.Create;//描述列表
  g_ExtractStringList := TStringList.Create;//扩展列表

{$IF CLIENTTEST = 0}
  LoadConfig(); //读取配置
{$ELSE}
  g_ConfigClient.WEffectImg := lmUseWzl;
  g_ConfigClient.WDragonImg := lmUseWzl;
  g_ConfigClient.WMainImages := lmUseWzl;
  g_ConfigClient.WMain2Images := lmUseWzl;
  g_ConfigClient.WMain3Images := lmUseWzl;
  g_ConfigClient.WChrSelImages := lmUseWzl;
  g_ConfigClient.WMMapImages := lmUseWzl;
  g_ConfigClient.WHumWingImages := lmUseWzl;
  g_ConfigClient.WHum1WingImages := lmUseWzl;
  g_ConfigClient.WHum2WingImages := lmUseWzl;
  g_ConfigClient.WBagItemImages := lmUseWzl;
  g_ConfigClient.WStateItemImages := lmUseWzl;
  g_ConfigClient.WDnItemImages := lmUseWzl;
  g_ConfigClient.WHumImgImages := lmUseWzl;
  g_ConfigClient.WHairImgImages := lmUseWzl;
  g_ConfigClient.WHair2ImgImages := lmUseWzl;
  g_ConfigClient.WWeaponImages := lmUseWzl;
  g_ConfigClient.WMagIconImages := lmUseWzl;
  g_ConfigClient.WNpcImgImages := lmUseWzl;
  g_ConfigClient.WFirNpcImgImages := lmUseFir;
  g_ConfigClient.WMagicImages := lmUseWzl;
  g_ConfigClient.WMagic2Images := lmUseWzl;
  g_ConfigClient.WMagic3Images := lmUseWzl;
  g_ConfigClient.WMagic4Images := lmUseWzl;
  g_ConfigClient.WMagic5Images := lmUseWzl;
  g_ConfigClient.WMagic6Images := lmUseWzl;
  g_ConfigClient.WHorseImages := lmUseWzl;
  g_ConfigClient.WHumHorseImages := lmUseWzl;
  g_ConfigClient.WHairHorseImages := lmUseWzl;
  for I := 0 to Length(g_ConfigClient.WMonImages) - 1 do
    g_ConfigClient.WMonImages[I] := lmUseWzl;
  LoadItemDescList; //载入物品描述
{$IFEND}

  LoadWMImagesLib(); //载入客户端图片资源 Data下
  ...
  Application.OnException := OnProgramException;//设置出错事件

  g_ProcessesList := TStringList.Create;  //创建进程列表

  //g_ShowFormList := TList.Create;
  m_MsgList := TGList.Create; //消息列表

  NpcImageList := TList.Create;//npc图片
  ItemImageList := TList.Create;//物品图片
  WeaponImageList := TList.Create;//武器图片
  HumImageList := TList.Create;//人类图片
  {g_DXSound := TDXSound.Create(Self);
  g_DXSound.Initialize;}
  DXDraw.Display.Width := SCREENWIDTH;
  DXDraw.Display.Height := SCREENHEIGHT;

  SoundEngine := TPlaySound.Create(Self);//声音引擎
  SoundEngine.Initialize;//初始化
  //无效混淆
  asm
   nop
   nop
  ...
  //键盘hook alt+tab 每次最小化间隔时间500毫秒
  g_ToolMenuHook := SetWindowsHookEx(WH_KEYBOARD, @KeyboardHookProc, 0, GetCurrentThreadId{线程id});
  //无效混淆
  asm
   nop
   nop
  ...
  g_SoundList := TStringList.Create;//声音列表
  BGMusicList := TStringList.Create;//背景音乐列表
  // 列表格式 1:	wav\1.wav
  flname := '.\wav\sound.lst';//声音列表文件
  //if not FileExists(flname) then
    //flname := '.\wav\sound.lst';

  LoadSoundList(flname);//载入声音

  flname := '.\wav\BGList.lst';//背景音乐文件
  LoadBGMusicList(flname);//载入背景音乐
  //if FileExists (flname) then
  //   SoundList.LoadFromFile (flname);
  DScreen := TDrawScreen.Create;//屏幕
  IntroScene := TIntroScene.Create;//介绍
  LoginScene := TLoginScene.Create;//登陆
  SelectChrScene := TSelectChrScene.Create;//选择角色
  PlayScene := TPlayScene.Create; //场景

  LoginNoticeScene := TLoginNotice.Create; //登陆公告

  Map := TMap.Create;//地图
  LegendMap := TLegendMap.Create;//地图

  g_MapDesc := TMapDesc.Create;//地图描述
  g_MapDesc.LoadFromFile(MAPDESC1FILE);//载入地图描述文件
  ...
  begin
    if Pos(g_sFontName, Screen.Fonts.Strings[I]) > 0 then begin
      g_sFontName := Screen.Fonts.Strings[I];//楷体
      break;
    end;
  end;
  ...
  g_nDayBright := 3;
  g_nAreaStateValue := 0;
  g_ConnectionStep := cnsStart;//连接步骤
  g_boSendLogin := False;
  ...
  BoOneClick := false;//必须为false 否则角色无法显示
  OneClickMode := toNone;
...
{$IFEND}
    ...
    with FrmDlg do begin
      ...
      for I := 0 to List.Count - 1 do begin //遍历列表
  ...
  if g_boFullScreen then begin //是否全屏模式    初始化游览器
    frmWelCome := TfrmWelCome.CreateParented(Handle);
    frmWelCome.Left := 0;
    frmWelCome.Top := 0;
  end else begin
    frmWelCome := TfrmWelCome.Create(nil);
    frmWelCome.SetStayOnTop;
  end;

//  if frmCqFirMediaPlayer = nil then begin
//    frmCqFirMediaPlayer := TfrmCqFirMediaPlayer.CreateParented(Handle);
...
//    if g_boFullScreen then begin
//      frmOpenDiaLogNew := TfrmOpenDiaLogNew.CreateParented(Handle);
...
begin
  ...
  if (not CanDraw) then begin
    ProcessKeyMessages;//使用魔法
    ProcessActionMessages;//解析，走，跑，骑马
    PlayScene.Run(True);//整个场景运行
...
{--------画底部健康提示----------------------------------------------------}
      ...
      if g_ConnectionStep <> cnsPlay then
        with ImageCanvas do   //版本号
          TextOut(TextHeight('A'), SCREENHEIGHT - TextHeight('A'), sVERSION, clGray);
{------------------------------------------------------------------------------}
      if g_ConfigClient.boShowFullScreen then begin  //是否显示切换全屏
...
begin
  ...
  case Key of
    ...
    善恶攻击模式：PK红名专用攻击模式。
      ...
      Word('K'): begin
     if not frmDlgConfig.Showing then frmDlgConfig.Open;
      end;
    Word('H'): begin
        if ssCtrl in Shift then begin
          SendSay('@AttackMode');
...
begin
  ...
  pt := ScreenToClient(pt);//将屏幕坐标转换为 窗体坐标
  DWinMan.Click(pt.X, pt.Y);
end;

procedure TfrmMain.MouseTimerTimer(Sender: TObject); //外挂
  ...
  var
    ...
    PickupList: TList;
  begin
    //if g_Config.boPickUpItem then begin //是否开启自动捡物
    if {(GetTickCount - g_dwPickUpItemTick > 300) and}(BagItemCount{背包物品数} < 46) then begin
      g_dwPickUpItemTick := GetTickCount;//拾取物品时间
      ...
      for I := 0 to PickupList.Count - 1 do begin
        if BagItemCount < 46 then begin
          if g_Config.boPickUpItemAll{是否全部拾取} then begin
            SendPickup;//发送捡取物品
          end else begin //否则就查找物品列表然后拾取
...
begin
  ...
  SetCursorPos(pt.X, pt.Y);//设置鼠标坐标

  if g_NewStatus > sNone then begin
    g_TargetCret := nil;
  end;

  if (g_MySelf <> nil) and (not g_MySelf.m_boDeath){没死} then begin
...
procedure TfrmMain.CmdTimerTimer(Sender: TObject); //命令时钟
begin
...
//            PlayScene.MemoLog.Lines.Add('ConnectionStep -1');
        ...
        g_ConnectionStep := cnsReSelChr;  //连接步揍为选择角色
        {
        except
          on e: Exception do
...
procedure TfrmMain.CSocketConnect(Sender: TObject;
  Socket: TCustomWinSocket); //这个sock可能切换到别的网关
var
...
begin
 ...
 { if g_ConnectionStep = cnsLogin then begin
  ...
  if g_ConnectionStep = cnsSelChr then begin //如果 连接步揍是选择角色
    //SelChrWaitTimer.Enabled := True;
    CmdTimer.Interval := 1;
    ActiveCmdTimer(tcFastQueryChr);//发送查询角色信息
    g_boGetRandomBuffer := True;
    SendGetRandomBuffer();//随机码
    LoginScene.OpenLoginDoor;
  end;
  if g_ConnectionStep = cnsReSelChr then begin //重新选择角色
    CmdTimer.Interval := 1;
    ActiveCmdTimer(tcFastQueryChr); //发送查询角色信息
...
begin
  g_boServerConnected := False;//服务器已连接为false
  //在开始和登陆状态 未发送登陆信息
  if (g_ConnectionStep in [cnsStart, cnsLogin]) and (not g_boSendLogin) then begin
    if not g_boShowVersionBmp then begin //没有显示LOGO图片
      FrmDlg.DMessageDlg('连接已经关闭...', [mbOk]);
      Close;
    end;
  end;
  if g_SoftClosed then begin  //小退游戏是真的
    g_SoftClosed := False;//改为假
    ActiveCmdTimer(tcReSelConnect); //重新连接选择角色服务器
  end;
end;
...
begin
  ...
  n := Pos('*', Data);
  if n > 0 then begin //如果有* 就去掉*并向服务器发送*s
    data2 := Copy(Data, 1, n - 1);
    Data := data2 + Copy(Data, n + 1, Length(Data));
    CSocket.Socket.SendText('*');//发送到服务端会被删除
  end;
  g_sSockText := g_sSockText + Data;//将收到的数据放入套接字
end;
...
begin
  ...
  SendSocket(EncodeMessage(Msg) + EncodeString(LoginID{用户名} + '/' + IntToStr(Certification{证书}))); //查询角色信息
end;
...
begin
  ...
  Busy := True;
  try   //处理包
    g_sBufferText := g_sBufferText + g_sSockText;//将接收到的数据移入缓存
    g_sSockText := ''; //清空接收到的数据

    if g_sBufferText <> '' then begin //判断是否有包
      mcnt := 0;
      while Length(g_sBufferText) >= 2 do begin
        if g_boMapMovingWait then Break;//如果正在放地图电影
        if Pos('!', g_sBufferText) <= 0 then Break;//判断是否有!
        g_sBufferText := ArrestStringEx(g_sBufferText, '#', '!', Data);//读取一条命令放入data
        if Data = '' then Break;
        DecodeMessagePacket(Data);//处理数据
  ...
  //查询价格
  if g_boQueryPrice then begin  //只查询一次
    if GetTickCount - g_dwQueryPriceTime > 500 then begin //每次查询需要 间隔500毫秒

      g_boQueryPrice := False;
      case FrmDlg.SpotDlgMode of //NPC售货对话框模式
        dmSell{销售模式}: SendQueryPrice(g_nCurMerchant, g_SellDlgItem.MakeIndex, g_SellDlgItem.S.Name);
        dmRepair:{修理模式} SendQueryRepairCost(g_nCurMerchant, g_SellDlgItem.MakeIndex, g_SellDlgItem.S.Name);
      end;
      OnClose := FormClose;
    end;
  end;
  ...
  if g_boShowVersionBmp and (GetTickCount - g_dwShowVersionBmpTick >= 1000 * 3) then begin
    ...
    if g_ConnectionStep in [cnsStart, cnsLogin] then begin
      if CSocket.Socket.Connected then begin  //判断socket是否连接

        SendClientMessage(CM_QUERYSERVERNAME, 0, 0, 0, 0);//查询服务器
        DScreen.ChangeScene(stLogin);//改变场景到登陆画面
        //获取外挂进程
        g_ProcessesList.Clear;
        if g_ConfigClient.sCheckProcessesUrl <> '' then begin
          HTTPGetCheckPro.URL := g_ConfigClient.sCheckProcessesUrl;
...
begin
  ...
  SendClientMessage(CM_QUERYBAGITEMS, 0, 0, 0, 0);//查询物品背包

  Msg := MakeDefaultMsg(CM_GETREGINFO, CSocket.Socket.Handle, 0, 0, 0);//获取注册信息
...
begin
  if boOpen and (sData <> '') then begin
    ...
    g_RandomSurface.TextOut(15 + Random(10), 0, DeCodeString(sData)); //$00050505
...
begin
  ...
  if sDataBlock[1] = '+' then begin
    ...
    ClientGetMsg(sTagStr);//关于技能 的打开关闭
  ...
  if g_MySelf = nil then begin
    case DefMsg.ident of
      ...
      SM_RANDOMCODE: ClientGetRandomCode(sBody, DefMsg.Recog);//获取验证码
      SM_NEWID_SUCCESS: ClientNewIDSuccess();//新建ID成功
      SM_NEWID_FAIL: ClientNewIDFail(DefMsg.Recog);//新建ID失败
      SM_PASSWD_FAIL: ClientLoginFail(DefMsg.Recog);//登陆失败
      SM_NEEDUPDATE_ACCOUNT: ClientGetNeedUpdateAccount(sBody);
      SM_UPDATEID_SUCCESS: ClientUpdateAccountSuccess();//帐号更新信息
      SM_UPDATEID_FAIL: ClientUpdateAccountFail();//帐号信息更新失败
      SM_PASSOK_SELECTSERVER: ClientGetPasswordOK(@DefMsg, sBody);//登陆成功后
      SM_SERVERNAME: ClientGetServerName(@DefMsg, sBody); //获取服务器列表
      SM_SELECTSERVER_OK: ClientGetPasswdSuccess(sBody); //显示登录框
      SM_QUERYCHR: ClientGetReceiveChrs(sBody);//获取角色信息
      SM_QUERYCHR_FAIL: ClientQueryChrFail(DefMsg.Recog);//查询角色失败
      SM_NEWCHR_SUCCESS: SendQueryChr();//发送查询角色信息
      SM_NEWCHR_FAIL: ClientNewChrFail(DefMsg.Recog);//创建角色失败
      SM_CHGPASSWD_SUCCESS: ClientChgPasswdSuccess();//密码修改成功
      SM_CHGPASSWD_FAIL: ClientChgPasswdFail(DefMsg.Recog);//密码修改失败
      SM_DELCHR_SUCCESS: SendQueryChr();//删除角色成功
      SM_DELCHR_FAIL: ClientDelChrFail(DefMsg.Recog);//删除角色失败
      SM_STARTPLAY: ClientGetStartPlay(sBody);//开始游戏 连接游戏网关 切换到公告显示
      SM_STARTFAIL: ClientStartPlayFail();//服务器满员
      //SM_VERSION_FAIL: ClientVersionFail();//游戏版本不符
      SM_FINDDELCHR: ClientGetDelChr(@DefMsg, sBody);//获取删除的角色
      SM_FINDDELCHR_SUCCESS: ClientGetDelChrOK(sBody);//获取删除角色成功
      SM_FINDDELCHR_FAIL: ClientGetDelChrFail(DefMsg.Recog);//获取ClientGetDelChr失败
      SM_SENDRANDOMCODE: ClientGetRandomDIB(DefMsg.Recog = 0, DefMsg.param, sBody); //获取验证码图片
      SM_OUTOFCONNECTION: ClientOutofConnection();//服务器中断
      SM_NEWMAP,
        SM_LOGON,
        SM_RECONNECT,
        SM_SENDNOTICE: ;
  ...
  nCode := 5;//处理游戏中数据
  case DefMsg.ident of
    //SM_VERSION_FAIL: ClientVersionFail();//游戏版本错误
    SM_NEWMAP: ClientGetNewMap(@DefMsg, sBody);
    SM_LOGON: ClientGetUserLogin(@DefMsg, sBody);//登陆

    SM_SERVERCONFIG: ClientGetServerConfig(@DefMsg, sBody);//获取服务器信息
    SM_RECONNECT: ClientGetReconnect(sBody);//重新连接
    SM_TIMECHECK_MSG: ;
   ...
   { SM_TAKEOFFTOHEROBAG_OK: ClientObjTakeOffHeroBagOK(DefMsg);
    ...
    SM_SENDOPENHOMEPAGE: BrowserNavigate(DecodeString(sBody));
...
begin
  sBody := DecodeString(sBody);
  //FrmDlg.DMessageDlg (sBody + '/' + IntToStr(DefMsg.Series), [mbOk]);
  nCount := _MIN(6, DefMsg.series); //求最小 如果超过6 只显示6个
  g_ServerList.Clear;
  for I := 0 to nCount - 1 do begin
    sBody := GetValidStr3(sBody, sServerName, ['/']);//获取服务器名称
    sBody := GetValidStr3(sBody, sServerStatus, ['/']);//获取服务器状态
    g_ServerList.AddObject(sServerName, TObject(Str_ToInt(sServerStatus, 0)));
  end;
  ClientGetSelectServer;
end;

procedure TfrmMain.ClientGetPasswordOK(DefMsg: pTDefaultMessage;//验证密码通过后
  sBody: string);
var
  Str, runaddr, runport, uid, certifystr: string;
begin
  LoginScene.OpenLoginDoor; //登陆场景 打开登陆门
  Str := DecodeString(sBody);
  Str := GetValidStr3(Str, runaddr, ['/']);//获取地址
  Str := GetValidStr3(Str, runport, ['/']);//端口
  Str := GetValidStr3(Str, certifystr, ['/']);//证明
  Certification := Str_ToInt(certifystr, 0);
  {Showmessage(runaddr);
   Showmessage(runport);}
  if not BoOneClick then begin
    CSocket.Active := False; //断开连接
    CSocket.Host := '';
    CSocket.Port := 0;
    FrmDlg.DSelServerDlg.Visible := False; //选择服务器隐藏
    //WaitAndPass(500); //WaitAndPass(500);
    g_ConnectionStep := cnsSelChr;//选择角色
    ...
    FrmDlg.DSelServerDlg.Visible := False;//选择服务器关闭
    g_sSelChrAddr := runaddr;
    g_nSelChrPort := Str_ToInt(runport, 0);

    if CSocket.Socket.Connected then  //判断是否已连接
      CSocket.Socket.SendText('$S' + runaddr + '/' + runport + '%');

    //WaitAndPass(500);
    g_ConnectionStep := cnsSelChr; //设置连接步骤到 选择角色
    LoginScene.OpenLoginDoor;
    SelChrWaitTimer.Enabled := True;//打开选择角色等待时钟
  end;
...
begin
  LoginScene.HideLoginBox;//隐藏登陆框
  FrmDlg.ShowSelectServerDlg;//显示选择服务器框
end;
...
begin
  with PlayScene do
    ...
    if g_FreeActorList <> nil then begin //需要释放的角色
...
function DelFileExt(sName: string): string;//删除文件扩展名
var
  nPos: Integer;
begin
  nPos := Pos('.', sName);//查找.是否存在
  if nPos > 0 then begin
    Result := Copy(sName, 1, nPos - 1);//反会字符串
  end else begin
    Result := sName;
  end;
end;

function FindProcesses(sName: string): Boolean;//查找进程
var
  I: Integer;
begin
  Result := False;
...
procedure TfrmMain.SpeedHackTimerTimer(Sender: TObject);//反外挂的
var
...
begin
  ...
  //通过服务器进程检测外挂 如果检测到进程就提示 速度异常 退出游戏
...
var
  ...
  DIB: TDIB;
begin
  g_MemoryStream := TMemoryStream.Create;//内存流
  g_MemoryStream.LoadFromFile(Application.ExeName);//载入客户端exe
{-------------------------------------------------------------------------------}
  nConfigClient := Length(EncodeBuffer(@ConfigClient, SizeOf(TConfigClient)));//取加密后长度
  g_MemoryStream.Seek(-nConfigClient, soFromEnd);//移动
  SetLength(sBuffer, nConfigClient);//设置长度
  g_MemoryStream.Read(sBuffer[1], nConfigClient);//读取
  DecodeBuffer(sBuffer, @g_ConfigClient, SizeOf(TConfigClient));//解密到g_ConfigClient

{-------------------------------------------------------------------------------}
  InBytes := g_ConfigClient.nSize;
  g_MemoryStream.Seek(0, soFromBeginning); //移动到首字节
  InBuf := g_MemoryStream.Memory;

{-------------------读取保存Data\F-Cqfir.Data-------------------------------------}
  if g_ConfigClient.nDataSize > 0 then begin
    InBytes := g_ConfigClient.nDataSize;
    GetMem(InBuf{指针}, InBytes);
    try
      g_MemoryStream.Seek(g_ConfigClient.nDataOffSet, soFromBeginning);
      g_MemoryStream.Read(InBuf^, InBytes);//读
      DecompressBuf(InBuf, InBytes, 0, OutBuf, OutBytes);//解密到outbuf
    finally
      FreeMem(InBuf);
    end;
    if OutBytes > 0 then begin
      sFileName := ExtractFilePath(Application.ExeName) + 'Data\F-Cqfir.Data';
      FileSetAttr(sFileName, 0);//设置权限
      MemoryStream := TMemoryStream.Create;
      MemoryStream.Write(OutBuf^, OutBytes);
      try
        MemoryStream.SaveToFile(sFileName);//保存到Data\F-Cqfir.Data
      except
...
begin
  ...
  for I := 0 to WeaponImageList.Count - 1 do begin
    TGameImages(WeaponImageList.Items[I]).Free;//武器
  end;
...
procedure TfrmMain.FindPathTimerTimer(Sender: TObject);//自动寻路
var
  ...
  if GetTickCount - g_dwPlayMediaTick > g_dwPlayMediaTime then begin
    ...
    try
      for I := 0 to g_MediaList.Count - 1 do begin
        MediaStartPlay(g_MediaList[I], False, True);
      end;
      g_MediaList.Clear;
    finally
      g_MediaList.UnLock;
