...
uses
  ...
  IntroScn, PlayScn, clEvent, MapUnit, GameImages, Wzl, Wis, Uib, Fir, Actor, Grobal2, IniFiles, SoundUtil,
...
type
  ...
  //lmUseWzl只读Wzl  lmUseData只读data  lmAutoWzl优先Wzl再data  lmAutoData优先data再Wzl
  TLoadMode = (lmUseWzl, lmUseFir, lmAutoWzl, lmAutoFir);
  TMirServer = record
    sServerName: string[30];//服务器名称
    sServeraddr: string[30];//服务器地址
    nServerPort: Integer;//服务器端口
    boFullScreen: Boolean;//是否全屏
    nScreenWidth: Integer; //屏幕宽度
    nScreenHegiht: Integer;//屏幕高度
    nClientCrc: Cardinal;//效验码
    nLoginCrc: Cardinal;//效验码
  end;
...
var
  ...
  g_boFullScreen: Boolean = True;//全屏模式
  ...
  g_WTiles2Images: TGameImages;
  g_WSmTilesImages: TGameImages;
  g_WSmTiles2Images: TGameImages;
...
//摆摊
  ...
  boOutbugStr: Boolean = True; //False; //调试模式是否开启
  ...
  g_nSelfThreadProcessId: Integer = 0;//自身进程ID
...
begin
  ...
  if (LoadMode = lmAutoFir) then begin //优先读取DATA文件 如果Data不文件存在则读取Wzl
    sFileName := ExtractFilePath(FileName) + ExtractFileNameOnly(FileName) + '.Data';
    if not FileExists(sFileName) then
      sFileName := ExtractFilePath(FileName) + ExtractFileNameOnly(FileName) + '.Wzl';
  end else
    if LoadMode = lmAutoWzl then begin //优先读取Wzl文件 如果Wzl文件不存在则读取DATA
    sFileName := ExtractFilePath(FileName) + ExtractFileNameOnly(FileName) + '.Wzl';
    if not FileExists(sFileName) then
      sFileName := ExtractFilePath(FileName) + ExtractFileNameOnly(FileName) + '.Data';
  end else
    ...
    if LoadMode = lmUseWzl then begin //只读取Wzl文件
    sFileName := ExtractFilePath(FileName) + ExtractFileNameOnly(FileName) + '.Wzl';
  end;
...
begin
  ...
  FileName := ExtractFilePath(Paramstr(0)) + 'Npc' + IntToStr(wAppr) + '.Wzl';
...
begin
  ...
  FileName := ExtractFilePath(Paramstr(0)) + 'Items' + IntToStr(FileIdx) + '.Wzl';
  if FileExists(FileName) then begin
...
begin
  ...
  FileName := ExtractFilePath(Paramstr(0)) + 'StateItem' + IntToStr(FileIdx) + '.Wzl';
  if FileExists(FileName) then begin
...
begin
  ...
  FileName := ExtractFilePath(Paramstr(0)) + 'Data\Weapon' + IntToStr(nAppr) + '.Wzl'; //Shape > 50  开始
...
begin
  ...
  FileName := ExtractFilePath(Paramstr(0)) + 'Data\Hum' + IntToStr(nAppr) + '.Wzl';
  if FileExists(FileName) then begin
...
begin
  //FillChar(WMonImagesArr, SizeOf(TWzlImages) * 100, 0);
...
var
  ...
  fhandle: TextFile;
begin
  if not boOutbugStr then Exit;//调试模式是否开启
  flname := '.\!debug.txt';//调试文件名
  if FileExists(flname) then begin //判断文件是否存在
    AssignFile(fhandle, flname); //关联文件
    Append(fhandle);//追加文本
  end else begin
    AssignFile(fhandle, flname);//关联
    Rewrite(fhandle);//新建文件
  end;
  Writeln(fhandle, DateTimeToStr(Now) + ' ' + Msg);//写入一行  时间+空格+消息
  CloseFile(fhandle); //关闭文件
end;
...
begin
  ...
  g_WEffectImg := GetGameImages(g_ConfigClient.WEffectImg, sFilePath + EFFECTIMAGEFILE);//特效
  g_WDragonImg := GetGameImages(g_ConfigClient.WEffectImg, sFilePath + DRAGONIMAGEFILE);//龙
  g_WMainImages := GetGameImages(g_ConfigClient.WMainImages, sFilePath + MAINIMAGEFILE);//主要1
  g_WMain2Images := GetGameImages(g_ConfigClient.WMain2Images, sFilePath + MAINIMAGEFILE2);//主要2
  g_WMain3Images := GetGameImages(g_ConfigClient.WMain3Images, sFilePath + MAINIMAGEFILE3);//主要3
  g_WUi1Images := GetGameImages(g_ConfigClient.WMainImages, sFilePath + UI1IMAGEFILE);//ui1
  ...
  g_WTilesImages := GetGameImages(lmUseWzl, sFilePath + TITLESIMAGEFILE);
  g_WTiles2Images := GetGameImages(lmUseWzl, sFilePath + TITLES2IMAGEFILE);
  g_WSmTilesImages := GetGameImages(lmUseWzl, sFilePath + SMLTITLESIMAGEFILE);
  g_WSmTiles2Images := GetGameImages(lmUseWzl, sFilePath + SMLTITLES2IMAGEFILE);
  ...
  g_WCqFirImages := GetGameImages(lmUseFir, sFilePath + CQFIRIMAGESFILE);//载入飞儿素材

  FillChar(g_WObjectArr, SizeOf(g_WObjectArr), 0);
  FillChar(g_WObjectArr16, SizeOf(g_WObjectArr16), 0);

  g_WMonImages := TMonImageList.Create;//怪物列表图片

  g_WHorseImages := GetGameImages(lmUseWzl, sFilePath + HORSEIMAGESFILE);//飞儿起码
  g_WHumHorseImages := GetGameImages(lmUseWzl, sFilePath + HUMHORSEIMAGESFILE);//骑马
  g_WHairHorseImages := GetGameImages(lmUseWzl, sFilePath + HAIRHORSEIMAGESFILE);//骑马发型
  g_WNpcFaceImages := GetGameImages(lmUseWzl, sFilePath + NPCFACEIMAGEFILE);//npc照片

  g_WBookImages := TUibImages.Create();//书
...
begin
  ...
  if g_WObjectArr[nUnit] = nil then begin
    ...
    g_WObjectArr[nUnit] := GetGameImages(lmUseWzl, sFileName);
