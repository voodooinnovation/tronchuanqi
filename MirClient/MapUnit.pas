...
uses
  Windows, Classes, SysUtils, Graphics, Grobal2, HUtil32, LoadMapThread, {DXDraws, } Share, MessageUnit;
...
// -------------------------------------------------------------------------------
  ...
  {TMapHeader = packed record
    ...
    TMapHeader =packed record //ÐÂÍÁ³Ç
     wWidth  : word;
     wHeight : word;
     btVersion:Byte;
     Title: string[13];
  ...
  TMapInfo_3 =  record //ÐÂÍÁ³Ç3
      wBkImg: word;
      wMidImg: word;
      wFrImg: word;
      btDoorIndex: byte;  //$80 (¹®Â¦), ¹®ÀÇ ½Äº° ÀÎµ¦½º
      btDoorOffset: byte;  //´ÝÈù ¹®ÀÇ ±×¸²ÀÇ »ó´ë À§Ä¡, $80 (¿­¸²/´ÝÈû(±âº»))
      btAniFrame: byte;      //$80(Åõ¸í)  ÇÁ·¡ÀÓ ¼ö
      btAniTick: byte;     //¸î¹ø¿¡ Æ½¸¶´Ù ÇÑ ÇÁ·¡ÀÓ¾¿ ¿òÁ÷ÀÌ´Â°¡
      btArea: byte;        //Object.Wzl ¹øÈ£
      btlight: byte;       //0..1..4 ±¤¿ø È¿°ú
      Tilse: word;
  end;
...
begin
  ...
  if FileExists(sFileName) then begin
    ...
    if nHandle > 0 then begin
      ...
      if Header.btVersion =15 then  m_boisMap3:=true else  m_boisMap3:=false;//ÅÐ¶ÏÊÇ·ñÐÂÍÁ
...
begin
  ...
  if FileExists(sFileName) then begin
    ...
    if nHandle > 0 then begin
      ...
      if Header.btVersion =15 then  m_boisMap3:=true else  m_boisMap3:=false;//ÅÐ¶ÏÐÂÍÁ
...
begin
  ...
  begin
    if bowalk then //°ÉÀ» ¼ö ÀÖÀ½
      ...
      m_MArr_3[cx, cy].wFrImg := m_MArr_3[cx, cy].wFrImg or $8000; //¸ø¿òÁ÷ÀÌ°Ô ÇÑ´Ù.
  end else
  begin
...
begin
  ...
  begin
    ...
    if Result then begin //¹®°Ë»ç
      if m_MArr_3[cx, cy].btDoorIndex and $80 > 0 then begin //¹®Â¦ÀÌ ÀÖÀ½
        if (m_MArr_3[cx, cy].btDoorOffset and $80) = 0 then
