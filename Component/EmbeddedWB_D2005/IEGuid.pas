(*******************************
 * IeGuid* (may 10 2005) Was created by : I don't know, Found it on the web *
...
uses
  clipbrd, Comobj, Activex, Windows, SysUtils, Classes, Forms, Dialogs;
...
begin
  ...
  if PropertyName = '' then
    ...
    Disp.GetIDsOfNames(GUID_NULL, @PName, 1, GetThreadLocale,
      @DispID);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (vt = VT_DISPATCH) or (vt = VT_PTR) then S.add(aname);
    end;
  ...
  begin
    ...
    begin
      if (SearchRec.Attr and $10 <> $10)
        then if Pos('.h', SearchRec.Name) > 0
        then FileList.Add(Path + '\' + SearchRec.Name);
    end;
    repeat
...
begin
  ...
  while t < I do
  begin
    if Copy(S, counter, 1) = Token then Inc(t);
    inc(counter);
  end;
  t1 := counter;
  if Copy(S, Counter, 1) = token then result := '' else
  begin
    Inc(Counter);
    while Copy(S, counter, 1) <> Token do Inc(counter);
    t2 := counter - t1;
    result := copy(S, t1, t2);
  end;
end;
...
begin
  ...
  repeat
    Temp := Findstr(S, X, ',');
    if (Length(Temp) = 1) or (Length(temp) = 3) or (Length(temp) = 7) then o := o + '0' + temp else
      o := o + temp;
    Inc(x);
  until Temp = '';
  s := o;
  ...
  if (Length(s) <> 38) or (Pos('name', g) > 0) or (Pos('DEFINE_', S) > 0) then result := '' else Result := g + '=' + S;
end;
...
begin
  ...
  while RegEnumKey(hk, dwindex, szIID, SizeOf(szIID)) = ERROR_SUCCESS
    ...
    begin
      ...
      if Guids.IndexOf(str) = -1 then Guids.Add(str);
    end;
    inc(dwIndex);
  end;
end;
...
//(1) Extract GUIDS from MIDL_INTERFACE("... lines in header files
      begin
        ...
        if n = '' then N := Copy(Trim(Lines[LCounter + 2]), 1, 255);
        n := Copy(n, 1, Pos(' ', n) - 1);
        S := n + '=' + g;
        if (Guids.IndexOf(S) < 0) and (n <> '') then Guids.Add(S);
      end
      else
...
//(2) Extract GUIDS from DEFINE_GUID("... lines in header files
          n := Lines[LCounter];
          x := LCounter;
          while (pos(');', n) = 0) and (x < lines.count) do begin
            Inc(x);
            n := n + Lines[x];
          end;
          S := ExtractDefineGuid(n);
          if s <> '' then
            if Guids.IndexOf(s) < 0 then Guids.Add(s);
        end
        else
...
begin
  ...
  begin
    ...
    if (pos('DISP', S) = 1) or (pos('HTML', S) = 1) or (pos('SID_', S) = 1) or
      ...
      then Temp.Add(Guids[i]) else
      if (pos('IID_I', S) = 1) and (Guids.IndexOf(Copy(S, 5, 255)) = -1) then Temp.add(copy(guids[i], 5, 255));
...
begin
  if Names <> nil then Names.Free;
  if Guids <> nil then Guids.Free;
  inherited;
end;
...
begin
  ...
  if i > -1 then
    Result := Names[i] else Result := GuidStr;
end;
...
begin
  ...
  if ((rsid <> '') and (x = -1)) or not Assigned(unk) then exit;
  if x > -1 then
  begin
    G := Guids[x];
    n := Names[x];
  ...
  if Succeeded(Unk.QueryInterface(IServiceprovider, isp))
    then for x := 0 to Guids.Count - 1 do begin
      if rsid = '' then
      begin
        G := Guids[x];
        N := Names[x];
