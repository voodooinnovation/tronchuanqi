...
uses
  ...
  JvComponentBase, ExtCtrls, RzBmpBtn;
...
procedure TfrmInspector.AddTest;
var
  MainInspCat,InspCat: TJvInspectorCustomCategoryItem;
  NewItem: TJvCustomInspectorItem;
begin

  MainInspCat := TJvInspectorCustomCategoryItem.Create(JvInspector1.Root, nil);
  MainInspCat.DisplayName := '按钮';

  InspCat := TJvInspectorCustomCategoryItem.Create(MainInspCat, nil);
  InspCat.DisplayName := '登陆游戏';

  TJvInspectorVarData.New(InspCat, '按钮', TypeInfo(TBitmap), Pointer(@RzBmpButton1.Bitmaps.Up));

  InspCat := TJvInspectorCustomCategoryItem.Create(JvInspector1.Root, nil);
  InspCat.DisplayName := '界面';
  TJvInspectorVarData.New(InspCat, '图片', TypeInfo(TBitmap), @Image1.Picture);

  InspCat := TJvInspectorCustomCategoryItem.Create(JvInspector1.Root, nil);
  InspCat.DisplayName := '颜色';
  TJvInspectorVarData.New(InspCat, '颜色', TypeInfo(TColor), @Color);
  TJvInspectorVarData.New(InspCat, '字体颜色', TypeInfo(TColor), @Font.Color);
end;
