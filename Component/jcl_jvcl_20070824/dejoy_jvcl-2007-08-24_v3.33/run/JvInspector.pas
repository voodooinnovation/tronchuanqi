...
-----------------------------------------------------------------------------}
// $Id: JvInspector.pas 11474 2007-08-18 13:57:48Z ahuser $
...
{$HPPEMIT ''}
  ...
  TJvInspectorTColorItem = class(TJvInspectorClassItem)
  protected
    procedure Edit; override;
    procedure SetFlags(const Value: TInspectorItemFlags); override;
  end;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jvcl.svn.sourceforge.net:443/svnroot/jvcl/trunk/jvcl/run/JvInspector.pas $';
    Revision: '$Revision: 11474 $';
    Date: '$Date: 2007-08-18 06:57:48 -0700 (Sat, 18 Aug 2007) $';
    LogPath: 'JVCL\run'
    );
{$ENDIF UNITVERSIONING}
...
{$ENDIF HAS_UNIT_VARIANTS}
  Consts, Dialogs, Forms, Buttons, ExtDlgs,
...
type
  TPictureEditorForm = class(TCustomForm)
  public
    grp: TGroupBox;
    mm: TImage;
    btnOK: TButton;
...
constructor TPictureEditorForm.CreateNew(AOwner: TComponent);
begin
  inherited CreateNew(AOwner);
  Caption := RsStringPictureEditorCaption;
  Width := 363;
  Height := 336;
  ...
  grp.Left := 12;
  grp.Top := (ClientHeight - grp.Height) div 2;

  grp.Anchors := [akTop, akLeft, akRight, akBottom];

  mm := TImage.Create(Self);
  mm.Parent := grp;
 ...
 // mm.OnChange := MemoChanged;
  ...
  btnLoad.Caption := RsButtonLoadCaption;
  btnLoad.Left := 10;
  btnLoad.Top := 250;
  btnLoad.OnClick := LoadPicture;

  btnSave := TButton.Create(Self);
  btnSave.Parent := grp;
  btnSave.Default := True;
  btnSave.Caption := RsButtonSaveCaption;
  btnSave.Left := 90;
  btnSave.Top := 250;
  ...
  btnClear.Parent := grp;
  btnClear.Cancel := True;
  btnClear.Caption := RsButtonClearCaption;
  btnClear.Left := 170;
  btnClear.Top := 250;
  btnClear.OnClick := ClearPicture;
...
procedure TPictureEditorForm.LoadPicture(Sender: TObject);
begin
  if OpenPictureDialog.Execute then begin
    mm.Picture.LoadFromFile(OpenPictureDialog.FileName);
    if (mm.Picture.Width > 0) and (mm.Picture.Height > 0) then
      mm.Stretch := (mm.Picture.Graphic.Width > mm.Width) or (mm.Picture.Graphic.Height > mm.Height);
  end;
end;

procedure TPictureEditorForm.SavePicture(Sender: TObject);
begin
  if SavePictureDialog.Execute then begin
    mm.Picture.SaveToFile(SavePictureDialog.FileName);
    if (mm.Picture.Width > 0) and (mm.Picture.Height > 0) then
      mm.Stretch := (mm.Picture.Graphic.Width > mm.Width) or (mm.Picture.Graphic.Height > mm.Height);
  end;
end;

procedure TPictureEditorForm.ClearPicture(Sender: TObject);
begin
  mm.Picture.Bitmap.Width := 0;
  mm.Picture.Bitmap.Height := 0;
  mm.Picture.Graphic.Width := 0;
  mm.Picture.Graphic.Height := 0;
  mm.Picture.Assign(mm.Picture.Graphic);
  mm.Picture.Assign(mm.Picture.Bitmap);
  mm.Repaint;
end;

//=== { TJvInspectorTPictureItem } ===========================================

constructor TJvInspectorTPictureItem.Create(const AParent: TJvCustomInspectorItem;
  const AData: TJvCustomInspectorData);
begin
  inherited Create(AParent, AData);
  ...
  if not Multiline then begin
    if Obj <> nil then begin
      if Obj is TPicture then begin
        if (TPicture(Obj).Width > 0) and (TPicture(Obj).Height>0) then begin
          if (TPicture(Obj).Bitmap <>nil) and (TPicture(Obj).Bitmap.Width > 0) and (TPicture(Obj).Bitmap.Height>0) then
            Result := Result + '(' + TPicture(Obj).Bitmap.ClassName + ')'
          else
            Result := Result + '(' + Obj.ClassName + ')';
        end else
          Result := Result + '(None)';
      ...
      if Obj is TBitmap then begin
        if (TBitmap(Obj).Width > 0) and (TBitmap(Obj).Height>0) then
          Result := Result + '(' + Obj.ClassName + ')'
        else
          Result := Result + '(None)';
      end else begin
        if (TGraphic(Obj).Width > 0) and (TGraphic(Obj).Height>0) then
          Result := Result + '(' + Obj.ClassName + ')'
        else
          Result := Result + '(None)';
      end;
    end else Result := Result + '(' + GetTypeData(Data.TypeInfo).ClassType.ClassName + ')';
  end else  Result := Obj.ClassName;
end;
...
var
  Obj: TObject;
begin
  with TPictureEditorForm.CreateNew(Inspector) do
  try
    Obj := TObject(Data.AsOrdinal);

    if Obj is TPicture then
      mm.Picture.Assign(TPicture(Obj))
    else if Obj is TBitmap then
      mm.Picture.Bitmap.Assign(TBitmap(Obj))
    else
      mm.Picture.Graphic.Assign(TGraphic(Obj));
    ...
    if ShowModal = mrOK then begin
      if Obj is TPicture then
        TPicture(Obj).Assign(mm.Picture.Graphic)
      else if Obj is TBitmap then
        TBitmap(Obj).Assign(mm.Picture.Graphic)
      else
        TGraphic(Obj).Assign(mm.Picture.Graphic);
...
procedure TJvInspectorTColorItem.Edit;
begin
  with TColorDialog.Create(GetParentForm(Inspector)) do
  try
    Color := TColor(Data.AsOrdinal);
   // Font.Assign(TFont(Data.AsOrdinal));
...
procedure TJvInspectorTColorItem.SetFlags(const Value: TInspectorItemFlags);
var
  NewValue: TInspectorItemFlags;
begin
  NewValue := Value + [iifEditButton, iifEditFixed];
...
begin
  ...
  begin
    // Add(TJvInspectorTypeInfoRegItem.Create(TJvInspectorTColorItem, TypeInfo(TColor)));
    ...
    Add(TJvInspectorTypeInfoRegItem.Create(TJvInspectorTPictureItem, TypeInfo(TPicture)));
    Add(TJvInspectorTypeInfoRegItem.Create(TJvInspectorTPictureItem, TypeInfo(TBitmap)));
  end;
end;
