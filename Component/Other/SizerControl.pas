...
//    Move, Copy or both TSizeControl actions
//21 Nov 2001
...
type
  ...
  TSizeControlEvent = procedure(Sender: TObject; ControlRect: TRect) of object;
  ...
  TSizeControl = class(TCustomControl)
  private
    fTarget: TControl;
    ...
    fOnMoving: TSizeControlEvent;
    fOnResizing: TSizeControlEvent;
    fOnMoved: TSizeControlEvent;
    fOnResized: TSizeControlEvent;
    fOnTargetChanged: TNewTargetEvent;
    fOnTargetChanging: TTargetChangingEvent;
    fOnSelectTarget: TNewTargetEvent;
    fOldFormWindowProc: TWndMethod;
    fOldTargetWindowProc: TWndMethod;
    procedure NewFormWindowProc(var Msg: TMessage);
    procedure TargetWindowProc(var Msg: TMessage);


    procedure HookFormWindowProc;
    procedure UnHookFormWindowProc;

    procedure HookTargetWindowProc;
    procedure UnHookTargetWindowProc;
...
{$ENDIF}
  ...
  published
    ...
    property OnMoving: TSizeControlEvent read fOnMoving write fOnMoving;
    property OnResizing: TSizeControlEvent read fOnResizing write fOnResizing;
    property OnMoved: TSizeControlEvent read fOnMoved write fOnMoved;
    property OnResized: TSizeControlEvent read fOnResized write fOnResized;
    ...
    property OnSelectTarget: TNewTargetEvent
      read fOnSelectTarget write fOnSelectTarget;
...
procedure Register;
begin
  RegisterComponents('Samples', [TSizeControl]);
end;
//------------------------------------------------------------------------------
...
var
  i: integer;
begin
  //nb: As TSizeControl does not assign its Parent property,
...
//------------------------------------------------------------------------------
// TSizeControl methods ...
//------------------------------------------------------------------------------

constructor TSizeControl.Create(aOwner: TComponent);
begin
  inherited;
  ...
  if not (aOwner is TCustomForm) then
    raise Exception.create('TSizeControl.create - Owner must be a TCustomFrom');
  fOwnerForm := TCustomForm(aOwner);
  fMyParent := fOwnerForm;
  //nb: Can't add TSizeControls to Parent's control list as that makes it
  //impossible for TSizeControl to be positioned above an aligned control ...
...
destructor TSizeControl.Destroy;
begin
...
procedure TSizeControl.HookFormWindowProc;
begin
...
procedure TSizeControl.UnHookFormWindowProc;
begin
...
procedure TSizeControl.HookTargetWindowProc;
begin
  if assigned(fTarget) then begin
    fOldTargetWindowProc := fTarget.WindowProc;
    fTarget.WindowProc := TargetWindowProc;
  end;
end;
//------------------------------------------------------------------------------

procedure TSizeControl.UnHookTargetWindowProc;
begin
  if assigned(fTarget) then begin
    fTarget.WindowProc := fOldTargetWindowProc;
  end;
end;
//------------------------------------------------------------------------------

procedure TSizeControl.CMChanged(var Message: TMessage);
begin
  if assigned(fOnTargetChanged) then fOnTargetChanged(self, fTarget);
end;
//------------------------------------------------------------------------------

procedure TSizeControl.CMNewTarget(var Message: TMessage);
var
...
procedure TSizeControl.TargetWindowProc(var Msg: TMessage);
var
  i, j: integer;
...
begin
  ...
  case Msg.Msg of
   ...
   { WM_SHOWWINDOW: SetWindowPos(Handle, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE + SWP_NOSIZE);
    //Handling TSizeControl's focus (using ParentWindow) is tricky!!
    ...
    WM_SIZE, WM_MOVE: if assigned(fTarget) then oldCoords := fTarget.BoundsRect;
   { WM_LBUTTONDOWN:
...
procedure TSizeControl.NewFormWindowProc(var Msg: TMessage);
...
procedure TSizeControl.CreateParams(var Params: TCreateParams);
begin
  inherited;
  if not (csDesigning in ComponentState) then
...
procedure TSizeControl.WMGetDlgCode(var Message: TMessage);
begin
  inherited;
...
procedure TSizeControl.WMEraseBkgnd(var Message: TMessage);
begin
  if (csDesigning in ComponentState) then
    inherited else
    Message.Result := 1;
...
procedure TSizeControl.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  if (csDesigning in ComponentState) then
...
function TSizeControl.DoTargetChanging(NewTarget: TControl): boolean;
begin
  result := true;
...
function TSizeControl.GetTargetAlign: TAlign;
begin
...
procedure TSizeControl.SetTargetAlign(NewAlign: TAlign);
begin
...
procedure TSizeControl.UpdateTargetCoords(MouseOp: TMouseOp);
begin
  invalidate;
...
function TSizeControl.GetTargetLeft: integer;
begin
...
procedure TSizeControl.SetTargetLeft(NewLeft: integer);
begin
...
function TSizeControl.GetTargetTop: integer;
begin
...
procedure TSizeControl.SetTargetTop(NewTop: integer);
begin
...
function TSizeControl.GetTargetWidth: integer;
begin
...
procedure TSizeControl.SetTargetWidth(NewWidth: integer);
begin
...
function TSizeControl.GetTargetHeight: integer;
begin
...
procedure TSizeControl.SetTargetHeight(NewHeight: integer);
begin
...
procedure TSizeControl.SetControlActions(ControlActions: TControlActions);
begin
...
procedure TSizeControl.KeyDown(var Key: Word; Shift: TShiftState);
...
procedure TSizeControl.KeyUp(var key: Word; Shift: TShiftState);
begin
  inherited;
  case key of
    VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT:
...
function TSizeControl.GetEnabled: boolean;
begin
...
procedure TSizeControl.SetEnabled(Enable: boolean);
begin
{$IFDEF VER100}
...
procedure TSizeControl.SetFocus;
begin
...
procedure TSizeControl.SetGridSize(GridSize: integer);
begin
...
procedure TSizeControl.SetTarget(target: TControl);
begin
  if fInternalChanging then fInternalChanging := false
  else if (fTarget = target) or (target is TSizeControl) then exit
  ...
  if fTarget <> nil then UnHookTargetWindowProc;
  ...
  //to position TSizeControl above aligned Targets ...
  parent := nil;
  fMyParent := fOwnerForm;
  ParentWindow := fMyParent.Handle;
...
procedure TSizeControl.Notification(AComponent: TComponent;
...
procedure TSizeControl.DrawRect;
var
  dc: hDC;
begin
...
procedure TSizeControl.WMKeyDown(var Message: TMessage);
begin
  //nb: GetParentForm() called in TWinControl.DoKeyDown() always returns nil
  //    because TSizeControl has no assigned Parent property, so...
...
procedure TSizeControl.WMKeyUp(var Message: TMessage);
begin
...
procedure TSizeControl.WMSetCursor(var Message: TWMSetCursor);
var
  pt: TPoint;
  rec: TRect;
  i: integer;
...
procedure TSizeControl.MouseDown(Button: TMouseButton;
...
begin
  ...
  if assigned(fOnSelectTarget) and assigned(fTarget) and PtInRect(fTarget.ClientRect, pt) then fOnSelectTarget(self, fTarget);
...
procedure TSizeControl.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  formOffset: TPoint;
...
procedure TSizeControl.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  pt: TPoint;
  rec: TRect;
begin
...
function TSizeControl.GetColor: TColor;
begin
  result := canvas.pen.color;
end;
//------------------------------------------------------------------------------

procedure TSizeControl.SetColor(Color: TColor);
begin
