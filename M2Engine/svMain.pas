...
{$IFEND}
    ...
    sCaptionExtText:=sCaptionExtText_Started;
    //Sleep(1000);
...
begin
  Randomize;
  g_dwGameCenterHandle := Str_ToInt(ParamStr(1), 0);//游戏中心句柄
  ...
  SendGameCenterMsg(SG_FORMHANDLE, IntToStr(Self.Handle));//发送句柄
  //设置网关列表参数
...
procedure TFrmMain.ReloadConfig(Sender: TObject); //重新加载配置
begin
  LoadConfig();
  FrmIDSoc.Timer1Timer(Sender);
  IdUDPClientLog.Host := g_Config.sLogServerAddr;//日志地址
  IdUDPClientLog.Port := g_Config.nLogServerPort;//端口
  LoadServerTable();
  LoadClientFile();
end;
...
procedure TFrmMain.MENU_CONTROL_RELOAD_CONFClick(Sender: TObject);//菜单 参数设置
begin
  ReloadConfig(Sender);
end;
...
{$IFEND}
 ...
 { g_Config.GlobalVal[496] := 1;
  ...
  RunSocket := TRunSocket.Create();//运行套接字
...
{$IFEND}
  ...
  MENU_OPTION_HERO.Visible := True;//现实英雄设置

  IdUDPClientLog.Host := g_Config.sLogServerAddr;//日志服务器地址
  IdUDPClientLog.Port := g_Config.nLogServerPort;//日志服务器端口
