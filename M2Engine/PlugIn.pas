...
begin
  ...
  sBuffer := EncodeString(IntToStr(nDeCodeString));
  Len := Length(sBuffer);
...
begin
  ...
  sBuffer := EncodeString(IntToStr(nIPLocal));
  Len := Length(sBuffer);
  Move(sBuffer[1], Buffer^, Len);
end;
...
begin
  ...
  sBuffer := EncodeString(IntToStr(nChangeCaptionText));
  Len := Length(sBuffer);
  Move(sBuffer[1], Buffer^, Len);
end;
...
begin
  ...
  sBuffer := EncodeString(IntToStr(nChangeGateSocket));
  Len := Length(sBuffer);
  Move(sBuffer[1], Buffer^, Len);
...
begin
  ...
  sBuffer := EncodeString(IntToStr(nGetMaxPlayObjectCount));
  Len := Length(sBuffer);
  Move(sBuffer[1], Buffer^, Len);
end;
...
begin
  ...
  sBuffer := EncodeString(IntToStr(nSetMaxPlayObjectCount));
  Len := Length(sBuffer);
  Move(sBuffer[1], Buffer^, Len);
end;
//==============================================================================
  ...
  if FileExists(sPlugLibFileName) then begin
    ...
    if Module > 32 then begin
      ...
      if @Init <> nil then begin
        ...
        PlugInfo.SysPlug^ := Str_ToInt(DeCodeString(Buffs), 0) = Module;
