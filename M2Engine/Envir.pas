...
type
  ...
  pTMap3UnitInfo = ^TMap3UnitInfo;
  TMap3 = array[0..1000 * 1000 - 1] of TMap3UnitInfo;
  pTMap3 = ^TMap3;
...
begin
  ...
  if FileExists(sMapFile) then begin
    ...
    if fHandle > 0 then begin
      ...
      //MessageBox(0,PChar(IntToStr(Header.btVersion)),'',0);
      ...
      begin
        ...
        nMapSize := m_nWidth * SizeOf(TMap3UnitInfo) * m_nHeight;

        Map3Buffer := AllocMem(nMapSize);
        FileRead(fHandle, Map3Buffer^, nMapSize);

        for nW := 0 to m_nWidth - 1 do begin
          n24 := nW * m_nHeight;
          for nH := 0 to m_nHeight - 1 do begin
            if (Map3Buffer[n24 + nH].wBkImg) and $8000 <> 0 then begin
              MapCellInfo := @MapCellArray[n24 + nH];
              MapCellInfo.chFlag := 1;
            end;
            if Map3Buffer[n24 + nH].wFrImg and $8000 <> 0 then begin
              MapCellInfo := @MapCellArray[n24 + nH];
              MapCellInfo.chFlag := 2;
            end;
            if Map3Buffer[n24 + nH].btDoorIndex and $80 <> 0 then begin
              Point := (Map3Buffer[n24 + nH].btDoorIndex and $7F);
